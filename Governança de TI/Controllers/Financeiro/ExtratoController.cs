...
public class ExtratoController : Controller
{
    ...
    public async Task<IActionResult> Index(DateTime? dataInicio, DateTime? dataFim, string periodo = "mes")
    {
        ...
        try
        {
            ...
            var despesasPorCat = listaParcelas
                   .Where(p => p.LancamentoPai.TipoLancamento == 1)
                   .GroupBy(p => p.LancamentoPai.Tipo != null ? p.LancamentoPai.Tipo.Nome : "Sem Categoria")
                   .ToDictionary(g => g.Key, g => g.Sum(x => x.ValorParcela));

            var receitasPorCat = listaParcelas
         .Where(p => p.LancamentoPai.TipoLancamento == 2)
         .GroupBy(p => p.LancamentoPai.Tipo != null ? p.LancamentoPai.Tipo.Nome : "Sem Categoria")
         .ToDictionary(g => g.Key, g => g.Sum(x => x.ValorParcela));


            // 5. MONTAGEM DO VIEWMODEL
            var model = new ExtratoViewModel
            {
                ...
                // [CORREÇÃO DE TIPO]
                // Parcelas: Recebe a lista de LancamentoParcelaModel
                Parcelas = listaParcelas,

                // Transacoes: Recebe a lista de LancamentoFinanceiroModel (Pais distintos)
                // O .Select(...).Distinct().ToList() garante a conversão correta
                Transacoes = listaParcelas.Select(p => p.LancamentoPai).Distinct().ToList(),

                DespesasPorCategoria = despesasPorCat,
                ReceitasPorCategoria = receitasPorCat
            };

            ViewBag.Contas = await _context.ContasBancarias.AsNoTracking().ToListAsync();
